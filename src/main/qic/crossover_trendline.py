...
# CONFIG
# ==========================
CSV_FILE   = "output/stocks_monthly.csv"           # input file (all symbols)
OUTPUT_DIR = "output"
...
def main():
    ...
    for symbol, group in df.groupby("Symbol"):
        result = compute_visual_angle(group)
        if result is None:
            continue

        angle_deg, (m, c), (x, y) = result
        ...
        # --------- Cross-up detection ----------
        # Trendline values for all points
        y_fit = m * x + c

        cross_up = False
        if len(y) >= 2:
            prev_diff = y[-2] - y_fit[-2]   # second last: close - trend
            last_diff = y[-1] - y_fit[-1]   # last: close - trend

            # Crossed from below (or touching) to above
        ...
        records.append(
            {
                ...
                "CrossUpTrend":    cross_up,
            }
        )

        # --------- Plot ONLY if cross-up ----------
        if cross_up:
    ...
    print(f"Plots (cross-up only) saved to: {PLOT_DIR}")

    # Optional: print quick list of cross-up stocks in console
    crossed = summary_df[summary_df["CrossUpTrend"] == True]
    if not crossed.empty:
        print("\nStocks where last close crossed UP above trend line:")
        for _, row in crossed.iterrows():
            print(f"{row['Symbol']}: angle={row['VisualAngleDeg']}°, downFromATH={row['DownFromATHPct']}%, RSI={row['RSI_Monthly']}")
    else:
        print("\nNo stocks crossed up above trend line.")
